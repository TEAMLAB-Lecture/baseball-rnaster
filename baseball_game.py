...
def is_digit(user_input_number):
    ...
    result = True
    for string in user_input_number:
        o = ord(string)
        if o < 48 or o > 57:
            result = False
            break
...
def is_between_100_and_999(user_input_number):
    ...
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당
    result = False
    if user_input_number[0] != "0" and 100 <= int(user_input_number) < 1000:
        result = True
    # ==================================
    return result
...
def main():
    ...
    while is_running:
        ...
        if is_digit(user_input) \
                and is_between_100_and_999(user_input) \
                and not is_duplicated_number(user_input):
